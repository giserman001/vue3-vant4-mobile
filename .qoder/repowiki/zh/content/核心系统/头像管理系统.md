...
- **重大更新**：新增npm脚本download:avatars和generate:group-avatar，简化头像管理流程
...
**更新** 本次更新重点改进了头像索引管理机制，实现了自动头像索引分配（avatarIdx），并优化了图片预加载性能。系统现在支持连续的头像编号分配，确保每个联系人在地址簿中的头像索引连续且可预测。**重大更新**：最小群组大小从3人减少到2人，可用头像配置从50个增加到100个，支持更多群组规模的头像组合，为用户提供更灵活的群组创建体验。**重大更新**：智能网格布局系统支持2-9人的群组头像合成，Sharp.js图像处理优化提供高性能的图片处理能力。**重大更新**：新增npm脚本download:avatars和generate:group-avatar，简化头像管理流程，提升开发效率。
...
- [user.ts:12-43](file://src/api/system/user.ts#L12-L43)
...
R --> T[Sharp图像处理]
U[download-avatars.js] --> V[Node FS]
W[package.json] --> X[脚本命令]
Y[circleFriendData.ts] --> Z[全局预加载]
AA[user.ts] --> BB[用户信息管理]
end
subgraph "状态管理依赖"
F --> CC[Pinia Store]
CC --> DD[Local Storage]
DD --> EE[Cache Management]
end
subgraph "工具依赖"
FF[新增npm脚本] --> GG[download:avatars]
FF --> HH[generate:group-avatar]
II[generate-group-avatar.js] --> JJ[Sharp Image Processing]
II --> KK[File System]
LL[package.json] --> MM[依赖包管理]
NN[circleFriendData.ts] --> OO[全局预加载]
PP[app.ts] --> QQ[联系人数据处理]
RR[AddChatModal.vue] --> SS[群组头像配置]
TT[generate-group-avatar.js] --> UU[头像生成配置]
VV[download-avatars.js] --> WW[头像下载配置]
end
```
...
**重大更新** 新增的npm脚本进一步优化了头像管理流程：

1. **download:avatars脚本**：自动化头像下载流程，支持批量下载和断点续传
2. **generate:group-avatar脚本**：自动化群组头像生成流程，支持100个不同头像的批量生成
3. **依赖管理**：通过package.json统一管理脚本依赖和版本控制
4. **错误处理**：完善的错误处理和日志记录机制
5. **配置灵活性**：支持自定义下载参数和生成配置
...
**重大更新** **问题描述**：download:avatars或generate:group-avatar脚本执行失败
...
9. **自动化脚本支持**：**重大更新**新增npm脚本，简化头像管理流程
...
10. ****重大更新****：新增npm脚本download:avatars和generate:group-avatar，提升开发效率
