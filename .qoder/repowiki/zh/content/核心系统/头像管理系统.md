...
**更新** 本次更新重点改进了头像索引管理机制，实现了自动头像索引分配（avatarIdx），并优化了图片预加载性能。系统现在支持连续的头像编号分配，确保每个联系人在地址簿中的头像索引连续且可预测。**重大更新**：最小群组大小从3人减少到2人，可用头像配置从50个增加到100个，支持更多群组规模的头像组合，为用户提供更灵活的群组创建体验。
...
G --> T[Pinia Store]
T --> U[Local Storage]
U --> V[Cache Management]
end
subgraph "工具依赖"
W[download-avatars.js] --> X[Node FS]
Y[generate-group-avatar.js] --> Z[Sharp Image Processing]
Y --> AA[File System]
AB[package.json] --> AC[脚本命令]
AD[circleFriendData.ts] --> AE[全局预加载]
AF[user.ts] --> AG[用户信息管理]
end
subgraph "配置依赖"
AH[package.json] --> AI[依赖包管理]
AJ[circleFriendData.ts] --> AK[全局预加载]
AL[app.ts] --> AM[联系人数据处理]
AN[AddChatModal.vue] --> AO[群组头像配置]
AP[generate-group-avatar.js] --> AQ[头像生成配置]
end
```
