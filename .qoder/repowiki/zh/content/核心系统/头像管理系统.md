...
**更新** 本次更新重点改进了头像索引管理机制，实现了自动头像索引分配（avatarIdx），并优化了图片预加载性能。系统现在支持连续的头像编号分配，确保每个联系人在地址簿中的头像索引连续且可预测。
...
subgraph "工具依赖"
P[download-avatars.js] --> Q[Node FS]
R[generate-group-avatar.js] --> S[Sharp Image Processing]
R --> T[File System]
...
subgraph "配置依赖"
V --> AA[依赖包管理]
AB[circleFriendData.ts] --> AC[全局预加载]
AD[app.ts] --> AE[联系人数据处理]
end
```
