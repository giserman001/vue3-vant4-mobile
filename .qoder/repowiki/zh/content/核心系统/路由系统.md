...
- [src/views/userProfile/index.vue](file://src/views/userProfile/index.vue)
...
- **新增**：用户资料路由配置，包括UserProfileSelf和UserProfileOther路由
...
B --> D["router/modules.ts<br/>模块化路由<br/>包含用户资料路由"]
...
V["views/userProfile/index.vue<br/>用户资料页面"] --> W["app.ts<br/>应用状态管理"]
C --> T
D --> V
```
...
- [src/router/modules.ts:169-189](file://src/router/modules.ts#L169-L189)
...
- [src/views/userProfile/index.vue:140-147](file://src/views/userProfile/index.vue#L140-L147)
...
- 模块化路由
  ...
  - **新增**：用户资料路由配置，支持个人资料与他人资料的统一页面组件
  - 参考路径：[src/router/modules.ts:5-146](file://src/router/modules.ts#L5-L146)、[src/router/modules.ts:169-189](file://src/router/modules.ts#L169-L189)
- 路由守卫
...
- **新增**：用户资料路由配置
  - UserProfileSelf：个人资料路由，无参数
  - UserProfileOther：他人资料路由，带 :id 参数
  - 统一使用 views/userProfile/index.vue 组件，通过路由参数区分显示模式
  - 支持从通讯录组件跳转到他人资料页面
  - 参考路径：[src/router/modules.ts:169-189](file://src/router/modules.ts#L169-L189)、[src/views/userProfile/index.vue:140-151](file://src/views/userProfile/index.vue#L140-L151)、[src/views/tabBar/components/address.vue:52-53](file://src/views/tabBar/components/address.vue#L52-L53)
- **新增**：错误页面路由配置
...
- [src/views/exception/ErrorPage.vue:1-59](file://src/views/exception/ErrorPage.vue#L1-L59)
- [src/views/exception/404.vue:1-41](file://src/views/exception/404.vue#L1-L41)
- [src/views/userProfile/index.vue:140-151](file://src/views/userProfile/index.vue#L140-L151)
- [src/views/tabBar/components/address.vue:52-53](file://src/views/tabBar/components/address.vue#L52-L53)
- [src/hooks/useRouteTransition.ts:13-61](file://src/hooks/useRouteTransition.ts#L13-L61)
- [src/hooks/useSwipeBack.ts:10-74](file://src/hooks/useSwipeBack.ts#L10-L74)
- [src/styles/transition/wx-slide.less:1-70](file://src/styles/transition/wx-slide.less#L1-L70)
...
## 架构总览
整体架构采用"常量路由 + 模块化路由"的组合模式，配合 Pinia 状态管理与导航守卫实现统一的权限与缓存策略。**新增的用户资料路由配置**提供了统一的个人资料与他人资料展示页面，支持通过路由参数区分显示模式。**新增的错误页面路由配置**提供了统一的错误处理机制，**新增的微信风格路由过渡系统**通过 useRouteTransition 和 useSwipeBack 提供自然流畅的移动端导航体验，**新增的 Tabbar 导航系统**通过 FloatingNavBar 组件提供现代化的移动端导航体验。
...
- **更新**：现在包含新增的 Tabbar 路由、错误页面路由和用户资料路由
- 参考路径：[src/router/index.ts:19-30](file://src/router/index.ts#L19-L30)
...
Router->>Router : "创建路由器并合并常量与模块化路由<br/>包含 Tabbar 路由、错误页面路由和用户资料路由"
...
- **新增**：用户资料路由配置
  - UserProfileSelf：个人资料路由，无参数，显示当前用户信息
  - UserProfileOther：他人资料路由，带 :id 参数，显示指定联系人信息
  - 统一使用 views/userProfile/index.vue 组件，通过路由参数区分显示模式
  - 支持从通讯录组件跳转到他人资料页面
  - **更新**：路由元信息扩展，支持 transition 属性控制过渡动画
- 参考路径：[src/router/modules.ts:5-146](file://src/router/modules.ts#L5-L146)、[src/router/modules.ts:169-189](file://src/router/modules.ts#L169-L189)、[types/router.d.ts:5-15](file://types/router.d.ts#L5-L15)
...
UserProfile["用户资料路由"] --> Self["UserProfileSelf<br/>个人资料"]
UserProfile --> Other["UserProfileOther<br/>他人资料"]
Self --> Component["views/userProfile/index.vue"]
Other --> Component
Component --> Computed["computed: isSelf<br/>通过路由参数判断"]
```
...
- **用户资料路由结构**
  ...
  - 统一使用 views/userProfile/index.vue 组件，通过路由参数区分显示模式
  - 支持从通讯录组件跳转到他人资料页面
  - 参考路径：[src/router/modules.ts:169-189](file://src/router/modules.ts#L169-L189)
- **用户资料页面组件**
  - views/userProfile/index.vue：统一的用户资料展示页面
  - 通过 computed 属性 isSelf 判断当前显示模式
  - 个人模式：显示当前用户信息，包含朋友圈和视频号内容
  - 他人模式：根据路由参数从通讯录数据中获取联系人信息
  - 支持发消息和音视频通话功能
  - 参考路径：[src/views/userProfile/index.vue:140-151](file://src/views/userProfile/index.vue#L140-L151)、[src/views/userProfile/index.vue:164-217](file://src/views/userProfile/index.vue#L164-L217)
- **应用状态管理**
  - app.ts：提供 findContactByAvatarIdx 方法查找联系人
  - provide enterChatByContact 方法创建或获取聊天
  - 参考路径：[src/store/modules/app.ts:552-561](file://src/store/modules/app.ts#L552-L561)、[src/store/modules/app.ts:569-607](file://src/store/modules/app.ts#L569-L607)
- **通讯录导航集成**
  - address.vue：通讯录组件中通过 /userProfile/:id 路由跳转
  - 支持从联系人列表直接进入他人资料页面
  - 参考路径：[src/views/tabBar/components/address.vue:52-53](file://src/views/tabBar/components/address.vue#L52-L53)
...
Self --> Component["views/userProfile/index.vue"]
Other --> Component
Component --> IsSelf["computed: isSelf<br/>判断显示模式"]
IsSelf --> |true| SelfInfo["selfInfo<br/>当前用户信息"]
IsSelf --> |false| OtherInfo["otherInfo<br/>联系人信息"]
SelfInfo --> Moments["朋友圈内容"]
SelfInfo --> Video["视频号内容"]
OtherInfo --> Phone["电话号码"]
OtherInfo --> Avatar["头像图片"]
Component --> Actions["操作按钮"]
Actions --> SendMessage["发消息"]
Actions --> Call["音视频通话"]
SendMessage --> Chat["enterChatByContact<br/>创建或获取聊天"]
```

**图表来源**
- [src/router/modules.ts:169-189](file://src/router/modules.ts#L169-L189)
- [src/views/userProfile/index.vue:140-151](file://src/views/userProfile/index.vue#L140-L151)
- [src/views/userProfile/index.vue:164-217](file://src/views/userProfile/index.vue#L164-L217)
- [src/views/tabBar/components/address.vue:52-53](file://src/views/tabBar/components/address.vue#L52-L53)
- [src/store/modules/app.ts:552-561](file://src/store/modules/app.ts#L552-L561)
- [src/store/modules/app.ts:569-607](file://src/store/modules/app.ts#L569-L607)
...
- **地址组件导航增强**
  - 地址组件中新增了跳转到错误页面的功能
  - 支持多个功能入口的错误页面导航
  - **新增**：支持跳转到用户资料页面，通过 /userProfile/:id 路由
  - 参考路径：[src/views/tabBar/components/address.vue:158-161](file://src/views/tabBar/components/address.vue#L158-L161)、[src/views/tabBar/components/address.vue:52-53](file://src/views/tabBar/components/address.vue#L52-L53)
- **Tabbar 导航系统**
  - **Tabbar 主页面**
...
Address --> ProfileNav["跳转到用户资料<br/>/userProfile/:id"]
...
- 路由器依赖
  - 常量路由与模块化路由共同构成完整路由表
  - **更新**：包含新增的 Tabbar 路由、错误页面路由和用户资料路由
...
- **新增**：用户资料路由依赖关系
  - 用户资料路由依赖 Layout 组件和统一的用户资料页面组件
  - 用户资料页面组件依赖应用状态管理中的通讯录数据
  - 通讯录组件依赖用户资料路由进行导航
  - 参考路径：[src/router/modules.ts:169-189](file://src/router/modules.ts#L169-L189)、[src/views/userProfile/index.vue:140-147](file://src/views/userProfile/index.vue#L140-L147)、[src/views/tabBar/components/address.vue:52-53](file://src/views/tabBar/components/address.vue#L52-L53)
- **新增**：错误页面路由依赖关系
...
- **新增**：Tabbar 依赖关系
  ...
  - 地址组件依赖错误页面路由和用户资料路由进行导航
  - 参考路径：[src/views/tabBar/index.vue:69-72](file://src/views/tabBar/index.vue#L69-L72)、[src/layout/components/FloatingNavBar.vue:88-90](file://src/layout/components/FloatingNavBar.vue#L88-L90)、[src/views/tabBar/components/address.vue:158-161](file://src/views/tabBar/components/address.vue#L158-L161)、[src/views/tabBar/components/address.vue:52-53](file://src/views/tabBar/components/address.vue#L52-L53)

```mermaid
graph LR
Router["router/index.ts"] --> Base["router/base.ts<br/>包含 Tabbar 路由和错误页面路由"]
Router --> Modules["router/modules.ts<br/>包含用户资料路由"]
...
Address --> ProfileRoute["UserProfileOther<br/>/userProfile/:id"]
...
ProfileRoute --> ProfilePage["userProfile/index.vue"]
ProfilePage --> AppStore["app.ts<br/>应用状态"]
```
...
- **新增**：用户资料路由性能优化
  - 用户资料页面组件采用懒加载，减少主包体积
  - 通过 computed 属性区分显示模式，避免重复渲染
  - 支持从通讯录数据中动态获取他人信息，无需额外请求
  - 参考路径：[src/router/modules.ts:178,188:178-178](file://src/router/modules.ts#L178-L178)、[src/router/modules.ts:188-188](file://src/router/modules.ts#L188-L188)、[src/views/userProfile/index.vue:140-151](file://src/views/userProfile/index.vue#L140-L151)
- **新增**：错误页面路由性能优化
  - 错误页面组件采用懒加载，减少主包体积
  - 支持嵌套子路由，避免重复代码
...
- [src/router/modules.ts:178,188:178-178](file://src/router/modules.ts#L178-L178)
- [src/router/modules.ts:188-188](file://src/router/modules.ts#L188-L188)
- [src/views/userProfile/index.vue:140-151](file://src/views/userProfile/index.vue#L140-L151)
...
- 缓存失效或不生效
  ...
  - 参考路径：[src/router/router-guards.ts:68-84](file://src/router/router-guards.ts#L68-L84)、[src/layout/index.vue:8-11](file://src/layout/index.vue#L8-L11)
- **新增**：用户资料路由问题
  - 检查 UserProfileSelf 和 UserProfileOther 路由配置是否正确
  - 确认 views/userProfile/index.vue 组件是否正确处理路由参数
  - 验证 isSelf 计算属性的逻辑是否正确
  - 检查通讯录数据中是否存在对应的联系人信息
  - 参考路径：[src/router/modules.ts:169-189](file://src/router/modules.ts#L169-L189)、[src/views/userProfile/index.vue:140-151](file://src/views/userProfile/index.vue#L140-L151)、[src/store/modules/app.ts:552-561](file://src/store/modules/app.ts#L552-L561)
- **新增**：错误页面路由问题
...
- **新增**：Tabbar 导航问题
  ...
  - 验证地址组件的错误页面导航和用户资料导航功能
  - 参考路径：[src/layout/components/FloatingNavBar.vue:208-220](file://src/layout/components/FloatingNavBar.vue#L208-L220)、[src/views/tabBar/index.vue:40-51](file://src/views/tabBar/index.vue#L40-L51)、[src/views/tabBar/components/address.vue:158-161](file://src/views/tabBar/components/address.vue#L158-L161)、[src/views/tabBar/components/address.vue:52-53](file://src/views/tabBar/components/address.vue#L52-L53)
...
## 结论
本项目的路由系统通过"常量路由 + 模块化路由"与 Pinia 状态管理相结合，实现了清晰的权限控制与高效的组件缓存策略。**新增的用户资料路由配置**提供了统一的个人资料与他人资料展示页面，支持通过路由参数区分显示模式，实现了个人中心与社交功能的完美结合。**新增的错误页面路由配置**提供了统一的错误处理机制，支持嵌套子路由和智能重试功能。**新增的微信风格路由过渡系统**通过 useRouteTransition 和 useSwipeBack 实现了前进/后退动画与手势滑动返回功能，提供了自然流畅的移动端导航体验。**新增的 Tabbar 导航系统**进一步增强了移动端用户体验，通过 FloatingNavBar 提供现代化的浮动导航体验，并在地址组件中集成了错误页面导航和用户资料导航功能。导航守卫统一处理登录拦截、白名单放行与缓存逻辑，配合 Layout 的 KeepAlive 实现良好的用户体验。整体架构易于扩展，适合在大型移动端应用中推广使用。
...
- 路由参数传递
  - 示例路径：[src/router/modules.ts:7-9](file://src/router/modules.ts#L7-L9)、[src/router/modules.ts:27-29](file://src/router/modules.ts#L27-L29)、[src/router/modules.ts:47-49](file://src/router/modules.ts#L47-L49)、[src/router/modules.ts:67-69](file://src/router/modules.ts#L67-L69)、[src/router/modules.ts:180-181](file://src/router/modules.ts#L180-L181)
- 登录守卫
...
- **新增**：用户资料路由配置
  - 示例路径：[src/router/modules.ts:169-189](file://src/router/modules.ts#L169-L189)、[src/views/userProfile/index.vue:140-151](file://src/views/userProfile/index.vue#L140-L151)、[src/views/userProfile/index.vue:164-217](file://src/views/userProfile/index.vue#L164-L217)、[src/views/tabBar/components/address.vue:52-53](file://src/views/tabBar/components/address.vue#L52-L53)
- **新增**：错误页面路由配置
