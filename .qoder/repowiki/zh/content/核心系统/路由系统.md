...
B --> C["router/base.ts<br/>常量路由定义<br/>包含 Tabbar 路由"]
...
- [src/router/base.ts:28-53](file://src/router/base.ts#L28-L53)
...
- [src/views/tabBar/index.vue:1-145](file://src/views/tabBar/index.vue#L1-L145)
...
- [src/styles/transition/wx-slide.less:1-69](file://src/styles/transition/wx-slide.less#L1-L69)
...
- 常量路由
  ...
  - 参考路径：[src/router/base.ts:28-53](file://src/router/base.ts#L28-L53)
- 模块化路由
...
- 布局与缓存
  ...
  - **新增**：FloatingNavBar 提供现代化的浮动 Tabbar 导航
...
- **新增**：微信风格路由过渡系统
  ...
  - 参考路径：[src/hooks/useRouteTransition.ts:13-61](file://src/hooks/useRouteTransition.ts#L13-L61)、[src/hooks/useSwipeBack.ts:10-74](file://src/hooks/useSwipeBack.ts#L10-L74)、[src/styles/transition/wx-slide.less:1-69](file://src/styles/transition/wx-slide.less#L1-L69)
- **新增**：Tabbar 导航系统
  - Tabbar 主页面：微信、通讯录、发现、我的四个核心页面
  - 浮动导航栏：响应式设计，支持主题切换和动画效果
  - 参考路径：[src/views/tabBar/index.vue:1-145](file://src/views/tabBar/index.vue#L1-L145)、[src/layout/components/FloatingNavBar.vue:1-549](file://src/layout/components/FloatingNavBar.vue#L1-L549)
...
## 架构总览
整体架构采用"常量路由 + 模块化路由"的组合模式，配合 Pinia 状态管理与导航守卫实现统一的权限与缓存策略。**新增的微信风格路由过渡系统**通过 useRouteTransition 和 useSwipeBack 提供自然流畅的移动端导航体验，**新增的 Tabbar 导航系统**通过 FloatingNavBar 组件提供现代化的移动端导航体验。
...
subgraph "路由层"
R["router/index.ts"]
B["router/base.ts<br/>包含 Tabbar 路由"]
U["router/modules.ts"]
...
Router->>Router : "创建路由器并合并常量与模块化路由<br/>包含 Tabbar 路由"
...
- **微信风格动画**
  ...
  - 参考路径：[src/styles/transition/wx-slide.less:1-69](file://src/styles/transition/wx-slide.less#L1-L69)
- **滑动返回手势**
  - 从屏幕左边缘 20px 内开始向右滑动
...
- **Tabbar 主页面**
  ...
  - 支持徽章显示、红点提醒等微信特色功能
  - 顶部导航栏与内容区域分离设计
  - 参考路径：[src/views/tabBar/index.vue:1-145](file://src/views/tabBar/index.vue#L1-L145)
...
- **新增**：Tabbar 依赖关系
  ...
  - 参考路径：[src/views/tabBar/index.vue:69-72](file://src/views/tabBar/index.vue#L69-L72)、[src/layout/components/FloatingNavBar.vue:88-90](file://src/layout/components/FloatingNavBar.vue#L88-L90)

```mermaid
graph LR
Router["router/index.ts"] --> Base["router/base.ts<br/>包含 Tabbar 路由"]
...
- **新增**：微信风格过渡动画性能优化
  ...
  - 参考路径：[src/styles/transition/wx-slide.less:5-69](file://src/styles/transition/wx-slide.less#L5-L69)
...
- [src/styles/transition/wx-slide.less:5-69](file://src/styles/transition/wx-slide.less#L5-L69)
...
- **新增**：微信风格过渡动画问题
  ...
  - 参考路径：[src/hooks/useRouteTransition.ts:38-61](file://src/hooks/useRouteTransition.ts#L38-L61)、[src/styles/transition/wx-slide.less:1-69](file://src/styles/transition/wx-slide.less#L1-L69)、[src/App.vue:5-5](file://src/App.vue#L5-L5)
- **新增**：滑动返回手势问题
  - 检查触摸事件监听是否正常
  - 验证边缘触发阈值和滑动比例设置
  - 确认 router.back() 调用时机
...
- **新增**：Tabbar 导航问题
  ...
  - 参考路径：[src/layout/components/FloatingNavBar.vue:208-220](file://src/layout/components/FloatingNavBar.vue#L208-L220)、[src/views/tabBar/index.vue:40-51](file://src/views/tabBar/index.vue#L40-L51)
...
## 结论
本项目的路由系统通过"常量路由 + 模块化路由"与 Pinia 状态管理相结合，实现了清晰的权限控制与高效的组件缓存策略。**新增的微信风格路由过渡系统**提供了自然流畅的移动端导航体验，通过 useRouteTransition 和 useSwipeBack 实现了前进/后退动画与手势滑动返回功能。**新增的 Tabbar 导航系统**进一步增强了移动端用户体验，通过 FloatingNavBar 提供现代化的浮动导航体验。导航守卫统一处理登录拦截、白名单放行与缓存逻辑，配合 Layout 的 KeepAlive 实现良好的用户体验。整体架构易于扩展，适合在大型移动端应用中推广使用。
...
- **新增**：微信风格路由过渡
  - 示例路径：[src/hooks/useRouteTransition.ts:13-61](file://src/hooks/useRouteTransition.ts#L13-L61)、[src/hooks/useSwipeBack.ts:10-74](file://src/hooks/useSwipeBack.ts#L10-L74)、[src/styles/transition/wx-slide.less:1-69](file://src/styles/transition/wx-slide.less#L1-L69)
- **新增**：Tabbar 导航系统
  - 示例路径：[src/views/tabBar/index.vue:1-145](file://src/views/tabBar/index.vue#L1-L145)、[src/layout/components/FloatingNavBar.vue:1-549](file://src/layout/components/FloatingNavBar.vue#L1-L549)、[src/layout/index.vue:43-48](file://src/layout/index.vue#L43-L48)
- **新增**：Tabbar 页面组件
