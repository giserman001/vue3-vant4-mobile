...
## 更新摘要
**变更内容**
- store模块重大重构：新增消息类型定义、聊天创建逻辑、问候消息池等聊天功能增强
...
- 新增完整的聊天状态管理：包括消息类型、聊天列表、联系人分组等
...
**重大更新**：store模块经过重大重构，新增了完整的聊天功能增强，包括消息类型定义、聊天创建逻辑、问候消息池等核心功能。
...
- 弹窗式交互设计，提供流畅的用户体验
- 完整的消息类型系统，支持普通消息和系统提示消息
- 智能的聊天创建逻辑，支持通过聊天列表和联系人入口创建新聊天
- 问候消息池，提供丰富的打招呼消息模板
...
- [src/store/modules/app.ts:23-40](file://src/store/modules/app.ts#L23-L40)
...
**重大更新**：store模块经过重大重构，提供了完整的聊天状态管理能力：

- **聊天状态管理**：处理聊天列表、消息数据、用户头像等
- **用户状态管理**：处理用户登录、登出、用户信息获取
- **设计设置状态管理**：处理主题设置、动画配置等
- **路由状态管理**：管理路由配置和组件缓存
- **联系人分组管理**：处理通讯录联系人分组和头像映射
- **消息类型定义**：定义标准的消息接口和类型
- **聊天创建逻辑**：支持通过多种入口创建新聊天
- **问候消息池**：提供丰富的打招呼消息模板
...
- [src/components/AddChatModal.vue:1-549](file://src/components/AddChatModal.vue#L1-549)
...
+sendMessage(id, content, isFriend) void
+toggleTop(id) void
+findContactByAvatarIdx(avatarIdx) ContactItem
+enterChatById(id) number
+enterChatByContact(name, avatar) number
}
class ChatMessage {
+content : string
+isFriend : boolean
...
AppStore --> GreetingMessagePool : "使用"
...
- [src/store/modules/app.ts:250-377](file://src/store/modules/app.ts#L250-L377)
- [src/store/modules/app.ts:22-38](file://src/store/modules/app.ts#L22-L38)
- [src/store/modules/app.ts:212-229](file://src/store/modules/app.ts#L212-L229)
...
flowchart TD
A[用户进入聊天] --> B{检查进入方式}
B --> |聊天列表入口| C[enterChatById]
B --> |联系人入口| D[enterChatByContact]
C --> E[清空未读计数]
E --> F[返回聊天ID]
D --> G[查找已有聊天]
G --> |找到| H[同步头像并清空未读]
...
**重大更新**：本次更新新增了AddChatModal组件和store模块的重大重构，这是一个功能强大、架构完善的聊天系统。新增的关键特性包括：
